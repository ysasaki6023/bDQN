...
#############
    def yieldBatch(self,nLength,nSteps,nBatch):
        while True:
            yield self.getBatch(nLength,nSteps,nBatch)

#############
    def getBatch(self,nLength,nSteps,nBatch):
        self.__loadFile( random.choice(self.inputPathList) ) # ランダムに1ファイルを読み込み。self系の変数にセット
        batchX = []
        batchT = []
        for i in range(nBatch):
            x,t = self.getOne(nLength,nSteps)
...
#############
    def getOne(self,nLength,nSteps):

        nWidth = int(nLength/nSteps)

        # すでに読み込まれているファイルからピックアップ
        self.posIdx = np.random.randint(0,len(self.min_asks_price)-nSteps*nWidth)

        x = np.concatenate([self.depth_asks[self.posIdx,:,0],
                            self.depth_bids[self.posIdx,:,0],
                            self.depth_asks[self.posIdx,:,1],
                            self.depth_bids[self.posIdx,:,1],
                            self.tseries   [self.posIdx,:  ]],axis=0)
