...
parser.add_argument("--initialJPY","-j",dest="initialJPY"  ,type=float,default=100000)
parser.add_argument("--learnRate",dest="learnRate"  ,type=float,default=1e-3)
parser.add_argument("--nStepsPerEpisode","-n",dest="nStepsPerEpisode"  ,type=int,default=2000) # 6分おきくらい
...
parser.add_argument("--agent","-a",dest="agent"  ,choices=["sarsa","dqn","dqnBoltz","duel","duelBoltz"],default="dqn")
parser.add_argument("--penalty","-p",dest="penalty"  ,type=float,default=5e-7) # 27分間くらいで流石に500円くらいは上がって欲しい。そうすると、-500/100000/10000=5e-7くらいになる
parser.add_argument("--batchSize","-b",dest="batchSize"  , type=int,default=3200)
parser.add_argument("--trainInterval","-t",dest="trainInterval"  , type=int,default=100)
